...
@section('content')
@php $currentType = old('pemilik_type', $lahan->pemilik_type ?? 'petani'); @endphp
        ...
        <div class="row">
            ...
            <div class="col-12 mb-3">
                ...
                <div class="btn-group btn-group-toggle" data-toggle="buttons">
                    <label class="btn btn-outline-primary {{ $currentType === 'petani' ? 'active' : '' }}" id="btn-petani">
                        <input type="radio" name="pemilik_type" value="petani" {{ $currentType === 'petani' ? 'checked' : '' }}>
                        <i class="fas fa-user-alt mr-1"></i> Petani / Anggota
                    </label>
                    <label class="btn btn-outline-success {{ $currentType === 'bumdes' ? 'active' : '' }}" id="btn-bumdes">
                        <input type="radio" name="pemilik_type" value="bumdes" {{ $currentType === 'bumdes' ? 'checked' : '' }}>
            ...
            {{-- Petani panel --}}
            <div class="col-md-6 mb-3" id="panel-petani" style="{{ $currentType === 'bumdes' ? 'display:none' : '' }}">
            ...
            {{-- BUMDes panel --}}
            <div class="col-md-6 mb-3" id="panel-bumdes" style="{{ $currentType === 'petani' ? 'display:none' : '' }}">
                ...
                <select name="bumdes_id" class="form-control" data-select2>
                    ...
                    @foreach($bumdes as $item)
                        <option value="{{ $item->id }}" {{ (string) old('bumdes_id', $lahan->bumdes_id) === (string) $item->id ? 'selected' : '' }}>
                            {{ $item->nama }}
                        </option>
                    @endforeach
                </select>
